...
//   //   try {
//   //     setSubmitting(true);
//   //     const response = await documentsPayableService.resultCheque(selectedCheque.id);
...
//     // درست: باید یک شیء با ساختار مورد نظر API ارسال بشه
//     const chequeData = {
//       id: selectedCheque.id,
//       chequeDateAccepts: null,
//       chequeDatePersianAccepts: null,
//       desc: "چک در سررسید پاس شده است",
//       typeResult: 1  // 1 = پاس شده
//     };
    
//     const response = await documentsPayableService.resultCheque(chequeData);
...
//     const chequeData = {
//       id: selectedCheque.id,
//       chequeDateAccepts: new Date(delayDate).toISOString(),
//       chequeDatePersianAccepts: convertToJalali(delayDate),
//       desc: delayReason,
//       typeResult: 1  // 2 = دیر پاس شده
//     };
...
//       1: { text: 'تأیید شده', color: '#10b981', bg: '#d1fae5', icon: '✅' },
//       2: { text: 'برگشت خورده', color: '#dc2626', bg: '#fee2e2', icon: '🚫' },
//     };
...
// DocumentsPayable.jsx - نسخه اصلاح شده با تاریخ دقیق
...
const DocumentsPayable = () => {
  ...
  useEffect(() => {
    ...
    if (showCalendar || showReturnCalendar || showEditCalendar) {
      document.addEventListener('mousedown', handleClickOutside);
    }
  ...
  }, [showCalendar, showReturnCalendar, showEditCalendar]);
  ...
  // ========== توابع اصلاح شده برای انتخاب تاریخ ==========
  
  // تابع انتخاب تاریخ برای فرم تاخیر - اصلاح شده
  const handleDateChange = (date) => {
    // اصلاح: استفاده از تاریخ محلی بدون تغییر منطقه زمانی
    const localDateStr = formatToLocalDate(date);
    setDelayDate(localDateStr);
    setShowCalendar(false);
  };

  // تابع انتخاب تاریخ برگشت خوردگی - اصلاح شده
  const handleReturnDateChange = (date) => {
    const localDateStr = formatToLocalDate(date);
  ...
  // تابع انتخاب تاریخ برای ویرایش - اصلاح شده
  const handleEditDateChange = (date) => {
    const localDateStr = formatToLocalDate(date);
    setEditFormData(prev => ({
      ...prev,
  ...
  const handleShowDelayForm = () => {
    setShowDelayForm(true);
    // استفاده از تاریخ محلی امروز
    setDelayDate(getTodayLocalDate());
  };
  ...
  const handleSubmitDelayed = async () => {
    ...
    try {
      ...
      // اصلاح: ساخت تاریخ ISO با استفاده از تاریخ محلی
      const [year, month, day] = delayDate.split('-');
      const localDate = new Date(parseInt(year), parseInt(month) - 1, parseInt(day));
      
      const chequeData = {
        id: selectedCheque.id,
        chequeDateAccepts: localDate.toISOString(),
        chequeDatePersianAccepts: convertToJalali(delayDate),
        desc: delayReason,
        typeResult: 1
      };
...
const handleSubmitReturned = async () => {
  ...
  try {
    ...
    // تبدیل تاریخ به فرمت ISO با حفظ تاریخ محلی
    const [year, month, day] = returnDate.split('-');
    const localDate = new Date(parseInt(year), parseInt(month) - 1, parseInt(day));
    
    // ساخت دیتا با فرمت مورد نظر API
    const chequeData = {
      id: selectedCheque.id,
      chequeDateAccepts: localDate.toISOString(),
      chequeDatePersianAccepts: convertToJalali(returnDate),
      desc: returnReason,
      typeResult: 2  // 2 = برگشت خورده (با توجه به فرمت شما)
    };
    
    console.log('📤 Sending return data:', chequeData);
    ...
    if (response?.data?.status === 200 || response?.status === 200) {
      ...
      throw new Error(response?.data?.message || 'خطا در ثبت');
    }
  } catch (err) {
    console.error('Error confirming cheque returned:', err);
    toast.error(err.message || 'خطا در ثبت اطلاعات', {
  ...
  const handleEdit = (cheque) => {
    setEditingCheque(cheque);
    // اصلاح: استفاده از تاریخ محلی برای نمایش در فرم
    const chequeDateValue = cheque.chequeDate ? cheque.chequeDate.split('T')[0] : '';
    setEditFormData({
      serialNumber: cheque.serialNumber || '',
      serialFinancail: cheque.serialFinancail || '',
      amount: cheque.amount || '',
                  ...
                  <div className="date-picker-wrapper" ref={calendarRef}>
                    ...
                    {showCalendar && (
                      <div className="calendar-dropdown">
                        <Calendar
                          onChange={handleDateChange}
                          value={delayDate ? parseLocalDate(delayDate) : new Date()}
                          locale="fa-IR"
                ...
                <div className="date-picker-wrapper" ref={returnCalendarRef}>
                  ...
                  {showReturnCalendar && (
                    <div className="calendar-dropdown">
                      <Calendar
                        onChange={handleReturnDateChange}
                        value={returnDate ? parseLocalDate(returnDate) : new Date()}
                        locale="fa-IR"
                      />
                    </div>
                  )}
              ...
              <div className="form-group">
                ...
                <div className="date-picker-wrapper" ref={editCalendarRef}>
                  ...
                  >
                    ...
                    <input
                      type="text"
                      value={editFormData.chequeDate ? convertToJalali(editFormData.chequeDate) : ''}
                      placeholder="انتخاب تاریخ"
                      readOnly
                      className="form-input date-input"
                      disabled={submitting}
                  ...
                  {showEditCalendar && (
                    <div className="calendar-dropdown">
                      <Calendar
                        onChange={handleEditDateChange}
                        value={editFormData.chequeDate ? parseLocalDate(editFormData.chequeDate) : new Date()}
                        locale="fa-IR"
                      />
                    </div>
                  )}
